...
$(document).ready(function () {
  ...
  const getDocuments = async () => {
    try {
      ...
      const request = await axios.get(
        "/admission/gestion/getdocuments/" + id_admission
      );
  ...
  const getInscriptionAnnee = async () => {
    ...
    try {
      icon.removeClass("fa-check").addClass("fa-spinner fa-spin");
      const request = await axios.get(
        "/admission/gestion/getAnneeDisponible/" + id_admission
      );
      const data = await request.data;
  ...
  $("body").on(
    "dblclick",
    "#datatables_gestion_admission tbody tr",
    function () {
      // const input = $(this).find("input");
      ...
      if ($(this).hasClass("active_databales")) {
        ...
        $("#datatables_gestion_admission tbody tr").removeClass(
          "active_databales"
        );
  ...
  $("body").on("click", ".ms-elem-selection", async function () {
    $(".ms-selectable .ms-list").prepend(
      $(this)
        .clone()
        .removeClass("ms-elem-selection")
    ...
    try {
      const request = await axios.post(
        "/admission/gestion/deletedocument",
        formData
      );
      const data = await request.data;
    } catch (error) {
  ...
  $("body").on("click", ".ms-elem-selectable", async function () {
    $(".ms-selection .ms-list").prepend(
      $(this)
        .clone()
        .removeClass("ms-elem-selectable")
    ...
    try {
      const request = await axios.post(
        "/admission/gestion/adddocuments",
        formData
      );
      const data = await request.data;
    } catch (error) {
  ...
  const getAdmissionInfos = async () => {
    try {
      ...
      const request = await axios.get(
        "/admission/gestion/info/" + id_admission
      );
      const data = await request.data;
  ...
  $("#save_frais_gestion").on("click", async () => {
    ...
    try {
      const request = await axios.post(
        "/admission/gestion/addfrais/" + id_admission,
        formData
      );
  ...
  $("#inscription_save").on("submit", async function (e) {
    ...
    try {
      const request = await axios.post(
        "/admission/gestion/inscription/" + id_admission,
        formData
      );
      const response = request.data;
  ...
  $("body").on("click", "#imprimer_docs", function () {
    ...
    window.open(
      "/admission/gestion/print_documents_admission/" + id_admission,
      "_blank"
    );
  });

  $("#inscription-masse").on("click", (e) => {
    e.preventDefault();
    // alert("hi");
  ...
  $("body").on("click", "#inscription_masse_enregistre", async function (e) {
    ...
    let modalAlert = $("#inscription_en_masse .modal-body .alert");

    modalAlert.remove();
    const icon = $("#inscription_masse_enregistre i");
    icon.removeClass("fa-check-circle").addClass("fa-spinner fa-spin");

    try {
      const request = await axios.post(
        "/admission/gestion/reinscription",
        formData,
        {
          headers: {
      ...
      const response = request.data;
      $("#inscription_en_masse .modal-body").prepend(
        `<div class="alert alert-success">
      ...
      console.log(error, error.response);
      modalAlert.remove();
      $("#inscription_masse_save .modal-body").prepend(
        `<div class="alert alert-danger">${message}</div>`
      );
      icon.addClass("fa-check-circle").removeClass("fa-spinner fa-spin ");
    }
