...
class HHonensRepository extends ServiceEntityRepository
{
    ...
    public function findHonoraireByCurrentYears($currentyear)
    {
        ...
        ens.code 'Code Enseignant',ens.nom 'Nom',ens.prenom 'Prenom',gr.id 'Id Grade',gr.designation 'Grade' ,hon.id 'Id Honoraire' ,
        CONCAT(FLOOR(HOUR(TIMEDIFF(emp.heur_fin, emp.heur_db)) / 10), MOD(HOUR(TIMEDIFF(emp.heur_fin, emp.heur_db)), 10), ':', FLOOR(MINUTE(TIMEDIFF(emp.heur_fin, emp.heur_db)) / 10), MOD(MINUTE(TIMEDIFF(emp.heur_fin, emp.heur_db)), 10)) 'Duree',IFNULL(hon.montant, 0)  'Montant', IFNULL(hon.brut, 0) 'Brut',IFNULL(hon.ir, 0) 'IR',ens.cin 'CIN', ens.rib 'RIB'
        FROM hhonens hon
        ...
        where ann.validation_academique = 'non' and ep.active = 1 ";
