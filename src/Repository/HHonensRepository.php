...
class HHonensRepository extends ServiceEntityRepository
{
    ...
    public function findHonoraireByCurrentYears($currentyear)
    {
        ...
        CONCAT(FLOOR(HOUR(TIMEDIFF(ep.heur_fin, ep.heur_db)) / 10), MOD(HOUR(TIMEDIFF(ep.heur_fin, ep.heur_db)), 10), ':', FLOOR(MINUTE(TIMEDIFF(ep.heur_fin, ep.heur_db)) / 10), MOD(MINUTE(TIMEDIFF(ep.heur_fin, ep.heur_db)), 10)) 'Duree',IFNULL(hon.montant, 0)  'Montant', IFNULL(hon.brut, 0) 'Brut',IFNULL(hon.ir, 0) 'IR',ens.cin 'CIN', ens.rib 'RIB'
        FROM hhonens hon
