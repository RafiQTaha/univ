...
class PlEmptimeRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    public function findSeanceByCurrentYears($currentyear)
    {
        ...
        DATE_FORMAT(date(start), '%Y/%m/%d') 'DateSeance', DATE_FORMAT(ep.start, '%H:%i') 'Date_début', DATE_FORMAT(ep.end, '%H:%i') 'Date_fin',time_to_sec(TIMEDIFF(ep.end,ep.start)) / 3600 'Volume Seance',ep.verifier, ep.valider,ep.generer,ep.annuler,ep.motif_annuler,s.id 'salle_id',s.code as code_salle,s.designation 'Salle',grp.id 'groupe_id',grp.niveau,epe.id 'enseignant_id',
