...
class GestionPreinscriptionController extends AbstractController
{
    ...
    {   
        ...
        $sheet->setCellValue('AD1', 'DATE PRE-INSCRIPTION');
        // $sheet->setCellValue('AB1', 'U-PREINS');
        // $sheet->setCellValue('AC1', 'U-REGLEMENT');
        $i=2;
        $j=1;
        $current_year = date('m') >= 6 ? $current_year = date('Y').'/'.date('Y')+1 : $current_year = date('Y') - 1 .'/' .date('Y');
        ...
        foreach ($preinscriptions as $preinscription) {
            ...
            foreach ($factures as $facture) {
                ...
                $sheet->setCellValue('AD'.$i, $preinscription->getCreated());
                $i++;
                $j++;
            }
            // if ($facture) {
